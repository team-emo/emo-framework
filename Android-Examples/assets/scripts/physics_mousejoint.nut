/*
 * Physics example by Box2D with dynamic body and MouseJoint.
...
class Main {
    ...
    function onLoad() {
        ...
        mouseJointDef.bodyB = boxInfo.getBody();
    ...
    /*
     * create mouse joint when user touches the screen.
     */
    function onMotionEvent(mevent) {
        if (mevent.getAction() == MOTION_EVENT_ACTION_DOWN && mouseJoint == null) {
            mouseJointDef.target = world.getWorldCoord(mevent.getX(), mevent.getY());
            mouseJoint = world.createJoint(mouseJointDef);
        } else if (mevent.getAction() == MOTION_EVENT_ACTION_MOVE && mouseJoint != null) {
            mouseJoint.setTarget(world.getWorldCoord(mevent.getX(), mevent.getY()));
        } else if (mevent.getAction() == MOTION_EVENT_ACTION_UP && mouseJoint != null) {
