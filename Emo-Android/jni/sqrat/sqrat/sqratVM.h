...
inline SQInteger SqratVM::runtimeErrorHandler(HSQUIRRELVM v)
{
    const SQChar *sErr = 0;
    if(sq_gettop(v) >= 1)
    {
...
inline void SqratVM::compilerErrorHandler(HSQUIRRELVM v,
    const SQChar* desc,
    const SQChar* source,
    SQInteger line,
    SQInteger column)
...
inline SqratVM::SqratVM(int initialStackSize /* = 1024 */)
...
inline void SqratVM::setPrintFunc(SQPRINTFUNCTION printFunc, SQPRINTFUNCTION errFunc)
{
    sq_setprintfunc(m_vm, printFunc, errFunc);
}

inline void SqratVM::setErrorHandler(SQFUNCTION runErr, SQCOMPILERERROR comErr)
{
...
inline SqratVM::ERROR_STATE SqratVM::doString(const Sqrat::string& str)
{
    Sqrat::string msg;
...
inline SqratVM::ERROR_STATE SqratVM::doFile(const Sqrat::string& file)
{
    Sqrat::string msg;
    m_lastErrorMsg.clear();
